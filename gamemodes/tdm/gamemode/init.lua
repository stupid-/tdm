...
function GM:PlayerDisconnected(ply)
    ...
    if (roundState == 2 and ply:Team() ~= TEAM_SPEC) then
...
function GM:PlayerSpawn(ply)
    ...
    elseif (ply:Team() == TEAM_RED and ply:GetPData("Player_Class", "noclass") ~= "noclass" or ply:Team() == TEAM_BLUE and ply:GetPData("Player_Class", "noclass") ~= "noclass") then
        ...
        timer.Simple(TDM_SpawnProtectionTime, function()
            unprotect(ply)
        end)

        hook.Add("KeyPress", "RemoveSpawnProtection", function(ply, key)
            if (key == IN_ATTACK and ply:HasGodMode() and ply:Alive()) then
                unprotect(ply)
            end
        end)
...
function PlayerAdvancedSpawnSelection(ply, spawnpointent, enemy_team)
    ...
    for k, v in pairs(EnemyEnts) do
        if (IsValid(v) and v:GetClass() == "player" and v:Alive() and v:Team() == enemy_team) then
            Blockers = Blockers + 1
        end
    end
...
--[[-------------------------------------------------------------------------
Neecs to be reworked || Spawns are important in overwatch
---------------------------------------------------------------------------]]
function GM:PlayerSelectSpawn(ply)
    ...
    if (playerTeam == TEAM_SPEC) then
        ...
        -- Found what we want, stop the loop
        ...
        if PlayerAliveCount <= 3 then
            for i = 0, SpawnPointCount do
                ...
                if (randomTeamSpawnPoint and randomTeamSpawnPoint:IsValid() and randomTeamSpawnPoint:IsInWorld() and randomTeamSpawnPoint ~= ply:GetVar("LastSpawnpoint") and randomTeamSpawnPoint ~= self.LastSpawnPoint) then
            ...
            if (closest and closest:IsValid() and closest:IsInWorld()) then
                return closest
            else
                for i = 0, SpawnPointCount do
                    randomTeamSpawnPoint = table.Random(Spawnpoints[playerTeam])
...
--[[-------------------------------------------------------------------------
Player Loadout, NEED TO BE LOOKED INTO FOR BACKEND
---------------------------------------------------------------------------]]
...
--[[-------------------------------------------------------------------------
All Functions on damage || Suicide is painless
---------------------------------------------------------------------------]]
...
function GM:PlayerShouldTakeDamage(ply, attacker)
    if (IsValid(attacker)) then
        if (attacker.Team and ply:Team() == attacker:Team() and ply ~= attacker and TDM_FriendlyFire == false) then
            return false
        elseif (attacker.Team and ply:Team() == attacker:Team() and ply ~= attacker and TDM_FriendlyFire == true) then
            return true
        end
    end
...
function stTeamSpec(ply)
    if (ply:Team() == TEAM_SPEC or ply.NextSwitchTime > CurTime()) then return end
...
function stTeamT(ply)
    ...
    if (ply:Team() == TEAM_RED or ply.NextSwitchTime > CurTime()) then return end
    --Protection against swapping and forcing an autobalance
    if (ply:Team() == TEAM_BLUE and RedPlayers == BluePlayers) then return end
...
function stTeamCT(ply)
    if (ply:Team() == TEAM_BLUE or ply.NextSwitchTime > CurTime()) then return end
    --Protection against swapping and forcing an autobalance
    if (ply:Team() == TEAM_RED and RedPlayers == BluePlayers) then return end
    --Players will not be screwed over in the last 45 seconds of the game
