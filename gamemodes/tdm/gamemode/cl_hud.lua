...
local function DrawXP(ply)
    ...
    for i = 0, 40 do
        if (i ~= 0 and i ~= 4 and i ~= 8 and i ~= 12 and i ~= 16 and i ~= 20 and i ~= 24 and i ~= 28 and i ~= 32 and i ~= 36 and i ~= 40) then
...
function NamesOverPlayers()
    if LocalPlayer():Team() == TEAM_SPEC then
        for k, v in pairs(player.GetAll()) do
            if v:IsValid() and v:Alive() and v:Team() ~= TEAM_SPEC then
...
function GM:HUDShouldDraw(name)
    if (name == "CHudHealth" or name == "CHudAmmo" or name == "CHudSecondaryAmmo") then return false end
...
hook.Add("PostDrawOpaqueRenderables", "PlayerNamesOverHeadsYeah", function()
    ...
    for k, v in pairs(player.GetAll()) do
        if (v:IsValid() and v:Alive() and (v:Team() ~= TEAM_SPEC) and (v:Team() ~= opposingTeam) and (v ~= LocalPlayer())) then
    ...
    for k, v in pairs(player.GetAll()) do
        if (v:IsValid() and v:Alive() and (v:Team() == opposingTeam) and (trace.Entity:IsPlayer()) and (v:Nick() == trace.Entity:Nick())) then
            local Pos = v:GetPos() + Vector(0, 0, 78)
            local Ang = (Pos - (LocalPlayer():EyePos())):Angle()
            cam.Start3D2D(Pos, Angle(0, Ang.y - 90, 90), 0.065)
